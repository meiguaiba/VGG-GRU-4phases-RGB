...
from keras.layers import Input, Dense, Flatten, Conv2D,concatenate, Lambda, GRU
...
def RGB_slice(x,index):
	...
	Arguments
		x: 12-channel images.
		index: the channel index of 12 images {0-2:RGB of plain,3-5:RGB of artery,6-8:RGB of venous, 9-11: RGB of delay}.
	Returns
		Gray scale images of selected channel. 
	"""
	x_gray1 = x[:, :, :, index]
	x_gray1 = K.reshape(x_gray1, (-1,224,224,1))
	x_gray2 = x[:, :, :, index+1]
	x_gray2 = K.reshape(x_gray2, (-1,224,224,1))
	x_gray3 = x[:, :, :, index+2]
	x_gray3 = K.reshape(x_gray3, (-1,224,224,1))
	x_gray = K.concatenate( [x_gray1, x_gray2, x_gray3], axis=-1)
...
def GRU_input(input):
	...
	Returns
		Concatenated tensor (None, 4, 128)
	"""
	x1 = input[0]
	x2 = input[1]
	x3 = input[2]
	...
	x4 = K.reshape(x4,(-1,1,128))
	encoder_concatenated = K.concatenate( [x1,x2,x3,x4], axis=-2 )

	return encoder_concatenated
	
def VGG_GRU_model(category, clinical_flag):
	"""
	...
	multi_image = Input(shape=(224, 224, 12))
	x_n = Lambda(RGB_slice,output_shape=(224,224,3),arguments={'index':0})(multi_image)
	x_a = Lambda(RGB_slice,output_shape=(224,224,3),arguments={'index':3})(multi_image)
	x_v = Lambda(RGB_slice,output_shape=(224,224,3),arguments={'index':6})(multi_image)
	x_d = Lambda(RGB_slice,output_shape=(224,224,3),arguments={'index':9})(multi_image)
	encoder_block = CNN_encoder()
	x_n = encoder_block(x_n)
	x_a = encoder_block(x_a)
	x_v = encoder_block(x_v)
	x_d = encoder_block(x_d)
	x_encoder= Lambda(GRU_input)([x_n, x_a, x_v, x_d])
	...
	if clinical_flag == True:
		...
		predictions = Dense(category, activation= 'softmax')(x_features)
